...
sns.set_context(rc={"figure.figsize": (9, 9)}, font_scale=2.0)
...
def load_data(data_path, embeddings_path, state=0):
    ...
    targets = np.array(
        [1 for entry in pos_vectors.index] + [-1 for entry in neg_vectors.index]
    )
...
def load_test_names(embeddings):
    ...
    NAMES_BY_ETHNICITY["White"] = [
        n.lower() for n in NAMES_BY_ETHNICITY["White"] if n.lower() in embeddings.index
    ]
    NAMES_BY_ETHNICITY["Black"] = [
        n.lower() for n in NAMES_BY_ETHNICITY["Black"] if n.lower() in embeddings.index
    ]

    white_female_start = NAMES_BY_ETHNICITY["White"].index("amanda")
    black_female_start = NAMES_BY_ETHNICITY["Black"].index("aiesha")

    test_gender = white_female_start * ["Male"] + (
        len(NAMES_BY_ETHNICITY["White"]) - white_female_start
    ) * ["Female"]
    test_gender += black_female_start * ["Male"] + (
        len(NAMES_BY_ETHNICITY["Black"]) - black_female_start
    ) * ["Female"]
    test_df = pd.DataFrame(
        {
            "name": NAMES_BY_ETHNICITY["White"] + NAMES_BY_ETHNICITY["Black"],
            "race": len(NAMES_BY_ETHNICITY["White"]) * ["White"]
            + len(NAMES_BY_ETHNICITY["Black"]) * ["Black"],
            "gender": test_gender,
        }
    )
...
def load_nyc_names(names_path, embeddings):
    ...
    names_df = names_df[
        np.logical_or(
            names_df["Ethnicity"] == "Black", names_df["Ethnicity"] == "White"
        )
    ]
...
def print_summary(test_df, method_name, test_accuracy):
    ...
    print(
        method_name
        + " race mean sentiment difference is %f\n"
    ...
    print(
        method_name
        + " gender mean sentiment difference is %f\n"
        % np.abs(mean_sentiments_gender[0] - mean_sentiments_gender[1])
    )

    fig, axs = plt.subplots(nrows=1, ncols=2, sharey=True, figsize=(6 * 2, 6))

    sns.boxplot(x="race", y=method_name + "_logits", data=test_df, ax=axs[0]).set_title(
        method_name, fontsize=20
    )
    sns.boxplot(
        x="gender", y=method_name + "_logits", data=test_df, ax=axs[1]
    ).set_title(method_name, fontsize=20)
...
def text_to_sentiment(text, network, embedding, device):
    ...
    with torch.no_grad():
        ...
        sentence_embeddings = (
            torch.cat(sentence_embeddings, dim=0).mean(dim=0, keepdim=True).to(device)
        )

        sentiment = network(sentence_embeddings)
        sentiment = torch.nn.functional.softmax(
            sentiment.mean(dim=0, keepdim=True), dim=-1
        )
