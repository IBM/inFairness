...
class SenSTIRAuditor():
  ...
  def __init__(self,
               ...
               distance_y: MahalanobisDistances, num_steps: int, lr: float, max_noise: float, min_noise: float):
  ...
  def generate_worst_case_examples(self, network, Q, lambda_param, optimizer):
    ...
    delta = Parameter( torch.rand_like(Q) * (self.max_noise - self.min_noise) + self.min_noise)
